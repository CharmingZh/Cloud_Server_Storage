...
int cp_r(int fd,int *workDir,char *buf,int uid,char *wDir,MYSQL *conn){
    ...
    sprintf(command,"select filename,md5,filesize,id from file where parent_id = %d \
            AND filename = '%s' AND type = %d",curDir,comBuf[2],0);
    res = selectCommand(conn,command);
    ...
    if(layer == 1){
        ...
        int toDir = 0;
        //先判断普通目标目录在当前目录下是否存在
        sprintf(command,"select id from file where parent_id = %d AND filename = \'%s\' AND type = 0",curDir,comBuf[3]);
        res = selectCommand(conn,command);
        row = mysql_fetch_row(res);
...
int dirCopy(int dirID,char *filename,int uid,int copyto,MYSQL *conn){
    ...
    MYSQL_ROW row;
    int inCopyto = 0;
    //先拷贝目录名，目录中的目录一定存在，拷贝也一定会成功
    sprintf(command,"insert into file (parent_id,filename,owner_id,md5,filesize,type) \
            values (%d,'%s',%d,NULL,NULL,%d)",copyto,filename,uid,0);
    mysqlCommand1(conn,command);
    //拷贝之后会有文件ID，获取此ID
    sprintf(command,"select id from file where parent_id = %d AND filename = '%s'",copyto,filename);
    res = selectCommand(conn,command);
    row = mysql_fetch_row(res);
    inCopyto = atoi(row[0]);
    //再判断目录下是否有文件
    sprintf(command,"select filename,md5,filesize,type,id from file where parent_id = %d",dirID);
    res = selectCommand(conn,command);
    while((row=mysql_fetch_row(res))!=NULL){//有文件就循环拷贝到目录
        if(atoi(row[3])==0){//如果目录中有目录
            dirCopy(atoi(row[4]),row[0],uid,inCopyto,conn);
        }
        else{//文件拷贝
            sprintf(command,"insert into file (parent_id,filename,owner_id,md5,filesize,type) \
                    values (%d,'%s',%d,'%s',%d,%d)",inCopyto,row[0],uid,row[1],atoi(row[2]),1);
            mysqlCommand1(conn,command);
        }
    }
    return 0;
