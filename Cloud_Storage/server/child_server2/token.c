...
int tokenCompare(MYSQL * conn,tokenClient_t tc){
    char buf[512] = {0};
    sprintf(buf,"select count(token) from tokenServer where token = '%s'",tc.token);
    ...
    if(now - atoi(row[0]) > EFFECTIVE_TIME){//超时
        memset (buf,0,sizeof(buf));
        sprintf(buf,"delete from tokenServer where token = '%s' and lasttime < %ld",tc.token,now-EFFECTIVE_TIME);
        mysqlCommand1(conn, buf);
        return tokenCompare(conn,tc);
    }else{//修改用户表的时间
        memset(buf,0,sizeof(buf));
